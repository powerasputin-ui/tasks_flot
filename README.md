...
## История / Time Travel (Phase 6, раздел 30 ТЗ)

* Страница `/history` — «Состояние на дату»: записи и их значения на выбранный день (только чтение, все роли).
* Восстанавливается откатом журнала изменений (`lib/time-travel.ts`); ограничение: до начала ведения журнала (в т.ч. до импорта из Excel) состояние неизвестно, такие даты показываются пустыми.

## Изменения за период / Change Engine (Phase 6, раздел 31 ТЗ)

* Страница `/changes` — что добавлено / изменено (Было → Стало) / архивировано за ISO-неделю или календарный месяц.
* Границы периода считаются по календарю (число дней в месяце, високосный год, ISO-неделя на стыке годов — `lib/change-engine.ts`); сравниваются два состояния Time Travel: на конец предыдущего и на конец выбранного периода.

## Canvas (Phase 6, раздел 93 ТЗ)

* Страница `/canvas`: карточки треков (по желанию — судов и задач) по «дорожкам» сегментов; карточка ссылается на запись, статус/привлекательность/ответственный читаются из основных таблиц.
* Куратор перетаскивает карточки — положение сохраняется в `canvas_items` (только `entityType/entityId/x/y`); остальные роли только смотрят. Допущение по правам — ANALYSIS.md, UNRESOLVED #6.
