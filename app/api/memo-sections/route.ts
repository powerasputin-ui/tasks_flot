...
import { loadFieldOptions, loadMemoConfig, loadSectionDefs, loadSources } from "@/lib/memo-load";
import { stripSectionHead } from "@/lib/memo";
...
async function GETHandler() {
  ...
  // трек/сегмент образца, ставший бы заголовком его раздела, из примера убираем — иначе пример разойдётся с настоящей справкой
  const head = sample ? stripSectionHead({ trackId: sample.trackId, segmentId: sample.segmentId, trackName: sample.trackName, segmentName: sample.segmentName }, defs) : null;
  ...
  return NextResponse.json({
    ...
    fields,
    sample: sample ? { title: sample.title, comment: sample.comment, segmentName: head!.segmentName, trackName: head!.trackName, ownerName: sample.ownerName, deadline: sample.deadline, statusName: sample.statusName, cost: sample.cost ?? null, attractivenessName: sample.attractivenessName ?? null, custom: sample.custom ?? {} } : null,
